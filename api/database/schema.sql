...
ALTER TABLE hubspot_custom_properties ENABLE ROW LEVEL SECURITY;

-- Sample RLS policies (adjust based on your auth strategy)
-- Users can only access data from their organization
CREATE POLICY "Users can access their organization's data" ON organizations
    FOR ALL USING (hubspot_portal_id = current_setting('app.current_portal_id')::BIGINT);

CREATE POLICY "Users can access their organization's contacts" ON contacts
    FOR ALL USING (organization_id IN (
        SELECT id FROM organizations 
        WHERE hubspot_portal_id = current_setting('app.current_portal_id')::BIGINT
    ));
